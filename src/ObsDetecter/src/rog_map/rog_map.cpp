...
ROGMap::ROGMap(const rclcpp::Node::SharedPtr& node) : node_(node) {

    cfg_ = rog_map::Config(node);
    initProbMap();
    ...
    if (cfg_.visualization_en) {
        vm_.occ_pub = node_->create_publisher<sensor_msgs::msg::PointCloud2>("rog_map/occ", 1);
        vm_.unknown_pub = node_->create_publisher<sensor_msgs::msg::PointCloud2>("rog_map/unk", 1);
        vm_.occ_inf_pub = node_->create_publisher<sensor_msgs::msg::PointCloud2>("rog_map/inf_occ", 1);
        vm_.unknown_inf_pub = node_->create_publisher<sensor_msgs::msg::PointCloud2>("rog_map/inf_unk", 1);

        if (cfg_.frontier_extraction_en) {
            vm_.frontier_pub = node_->create_publisher<sensor_msgs::msg::PointCloud2>("rog_map/frontier", 1);
        }

        if (cfg_.esdf_en) {
            vm_.esdf_pub = node_->create_publisher<sensor_msgs::msg::PointCloud2>("rog_map/esdf", 1);
            vm_.esdf_neg_pub = node_->create_publisher<sensor_msgs::msg::PointCloud2>("rog_map/esdf/neg", 1);
            vm_.esdf_occ_pub = node_->create_publisher<sensor_msgs::msg::PointCloud2>("rog_map/esdf/occ", 1);
        }

        if (cfg_.viz_time_rate > 0) {
            vm_.viz_timer = node_->create_wall_timer(std::chrono::duration<double>(1.0 / cfg_.viz_time_rate), 
                                                    std::bind(&ROGMap::vizCallback, this));
        }
    }
    vm_.mkr_arr_pub = node_->create_publisher<visualization_msgs::msg::MarkerArray>("rog_map/map_bound", 1);

    if (cfg_.ros_callback_en) {
        rc_.odom_sub = node_->create_subscription<nav_msgs::msg::Odometry>(
            cfg_.odom_topic, 1, std::bind(&ROGMap::odomCallback, this, std::placeholders::_1));
        rc_.cloud_sub = node_->create_subscription<sensor_msgs::msg::PointCloud2>(
            cfg_.cloud_topic, 1, std::bind(&ROGMap::cloudCallback, this, std::placeholders::_1));
        rc_.update_timer = node_->create_wall_timer(std::chrono::milliseconds(1), 
                                                   std::bind(&ROGMap::updateCallback, this));
    }
...
void ROGMap::updateRobotState(const Pose& pose) {
    ...
    robot_state_.rcv_time = node_->now().seconds();
...
void ROGMap::odomCallback(const nav_msgs::msg::Odometry::SharedPtr odom_msg) {
    ...
    static tf2_ros::TransformBroadcaster br_map_ego(node_);
...
void ROGMap::cloudCallback(const sensor_msgs::msg::PointCloud2::SharedPtr cloud_msg) {
    if (!robot_state_.rcv) {
        return;
    }
    double cbk_t = node_->now().seconds();
...
void ROGMap::updateCallback() {
    if (map_empty_) {
        static double last_print_t = node_->now().seconds();
        double cur_t = node_->now().seconds();
